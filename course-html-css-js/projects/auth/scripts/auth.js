...
const isValidLength = (str, min, max = Infinity) => str.length >= min && str.length <= max;
...
const isNameValid = (name) =>
  ...
  name
    .split('')
    .every((symbol) => (symbol >= 'a' && symbol <= 'z') || (symbol >= 'A' && symbol <= 'Z'));
...
registerForm.addEventListener('submit', (event) => {
  ...
  if (
    ...
    console.log(`// Поздравляем, ${newUser.name}! Вы успешно зарегистрировались`);
...
loginForm.addEventListener('submit', (event) => {
  ...
  if (isEmailValid(email) && isLoginValid(email, password, users)) {
    ...
    localStorage.setItem('authToken', users.filter((user) => user.email === email)[0].token);
